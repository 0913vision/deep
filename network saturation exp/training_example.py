...
class Trainer:
    ...
    def _run_epoch (self, epoch):
        ...
        print(f"[GPU{self.global_rank}] Epoch {epoch} | n_duration = {val_end_time - start_time} | t_duration = {train_end_time - start_time} | v_duration = {val_end_time - train_end_time} | avg_train_loss = {train_loss} | avg_validation_loss = {val_loss} | validation_accuracy = {val_accuracy}")
...
def load_train_objs ():
    ...
    training_model = models.resnet152(num_classes=10)
...
def main (save_period: int, total_epochs: int, batch_size: int, snapshot_path: str, train_node: DASH.TRAIN):
    ...
    print(f"[{int(os.environ['RANK'])}] [makespan] [{time.time()-start_time}]")
