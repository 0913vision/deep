...
This package provides the DASH framework for distributed deep learning. It includes classes and functions necessary for utilizing DASH in your learning script.

To use DASH, import the package into your learning script and utilize the package through the DASH_init() function.
...
class MPI:
    ...
    def _recv(self, source:int, tag:int, shard_rank:int, save_count:int) -> bytes:
        ...
        data_buffer_size = c_int()
        self.__recv(self.__mpi_communication, c_int(int(source)), c_int(int(tag)), byref(data_buffer), byref(data_buffer_size), c_int(shard_rank), c_int(save_count))
...
class TRAIN:
    ...
    class __CopierClass:
        ...
        def __init__(
            ...
            self.__buffer = None
            self.__current_save = 0

        def __method(self):
            for i in range(self.__save_count):
                self.__current_save = self.__current_save+1
                ...
                print(f"[{self.__shard_rank}] [{i+1}] [serialization] [{time.time()}]")
        ...
        def wait_copy_complete(self):
            ...
            while not self.__copy_completed:
                time.sleep(0.01)
            print(f"[{self.__shard_rank}] [{self.__current_save}] [CST] [{time.time()-start_time}]")
    ...
    def save(self, obj):
        ...
        if self.__shard_rank >= 0:
            print(f"[{self.__shard_rank}] [{self.i+1}] [start] [{time.time()}]")
    ...
    class __ReceiverClass:
        ...
        def __method(self):
            for i in range(self.__save_count):
                ...
                received_data = self.__MPI.recv(source=self.__MPI.sharding_rank_list[self.__rank], tag=0, deserialize=False, shard_rank=self.__rank, save_count=i+1)
